...
export default function PopularCourses({ courses = [], title = "জনপ্রিয় কোর্স" }) {
  ...
  return (
    <section className="space-y-3 px-4">
      <div>
        <h3 className="text-base font-bold text-[var(--color-text-dark)]">{title}</h3>
      ...
      <div className="-mx-4 overflow-x-auto pb-2 pl-4 pr-4">
        <div className="flex gap-4 touch-pan-x overflow-x-auto">
          {items.map((course, idx) => (
            <div key={idx} className="min-w-[260px] flex-shrink-0 rounded-[16px] bg-white p-4 shadow-sm ring-1 ring-[var(--color-border)]">
              ...
              <div className="mt-3 inline-flex rounded-full bg-[var(--color-bg-light)] px-3 py-1 text-sm font-semibold text-[var(--color-primary-green)]">
                {course.duration ? `মেয়াদ: ${course.duration}` : course.category || ""}
              </div>
              <div className="mt-4">
